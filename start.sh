...
if [ "$PROCESS_TYPE" = "worker" ]; then
    ...
    # We start a tiny health check responder (V3) to satisfy the port scan.
    echo "Starting health check responder (V3) on port $PORT..."
    python3 -c 'from http.server import HTTPServer, BaseHTTPRequestHandler; HTTPServer(("0.0.0.0", '"$PORT"'), type("H", (BaseHTTPRequestHandler,), {"do_GET": lambda s: (s.send_response(200), s.end_headers(), s.wfile.write(b"OK")), "do_HEAD": lambda s: (s.send_response(200), s.end_headers()), "log_message": lambda *a: None})).serve_forever()' &
